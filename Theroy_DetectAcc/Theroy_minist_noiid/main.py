...
from Theroy_minist_noiid.attention import Attention
import Theroy_minist_noiid.datasource
from Theroy_minist_noiid.models import GlobalModel, GlobalModel_MNIST_CNN, RESNET18_MODEL, LENET5_MODEL, LENET5_MODEL_FEMNIST
from Theroy_minist_noiid.fl_client import FederatedClient
from Theroy_minist_noiid.datasource import Mnist
    ...
    def handle_client_update(self, data):
        ...
        if self.aggregation in ["normal_atten", "atten", "rule_out"]:
            ...
            if self.aggregation == "normal_atten":
                ...
                attention = self.attenpiption_mechanism.cal_weights(np.array( uploaded_weights ))
    ...
    if train:
        if iid:
            with open(aggregation + '_minist_iid_train_acc_结果存放.txt', 'a') as file_handle:
                ...
                file_handle.write('\n')
            with open(aggregation + '_minist_iid_train_loss_结果存放.txt', 'a') as file_handle:
                file_handle.write(str(score[0]))
                file_handle.write('\n')
        else:
            with open(aggregation + '_minist_noniid_train_acc_结果存放.txt', 'a') as file_handle:
                file_handle.write(str(score[1]))
                file_handle.write('\n')
            with open(aggregation + '_minist_noniid_train_loss_结果存放.txt', 'a') as file_handle:
                file_handle.write(str(score[0]))
                file_handle.write('\n')
    else:
        if iid:
            with open(aggregation + '_minist_iid_test_acc_结果存放.txt', 'a') as file_handle:
                file_handle.write(str(score[1]))
                file_handle.write('\n')
            with open(aggregation + '_minist_iid_test_loss_结果存放.txt', 'a') as file_handle:
                file_handle.write(str(score[0]))
                file_handle.write('\n')
        else:
            with open(aggregation + '_minist_noniid_test_acc_结果存放.txt', 'a') as file_handle:
                file_handle.write(str(score[1]))
                file_handle.write('\n')
            with open(aggregation + '_minist_noniid_test_loss_结果存放.txt', 'a') as file_handle:
                file_handle.write(str(score[0]))
                file_handle.write('\n')
    return score
...
#                             attack_mode[selected_idx], writer=writers[ selected_writers[selected_idx] ]) )
        ...
        test_loss[cur_round] = score_test[0]
